...
public class ball : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetMouseButtonUp(0))
        {
            ...
            if (!ClickOnGUI(Input.mousePosition) && !launched && !ball.GetComponent<ball>().setTarget &&
                MainScript.Instance.newBall2 == null /*&& MainScript.Instance.newBall == null*/ && newBall &&
        ...
        if (transform.position != target && setTarget && !stopedBall && !isPaused &&
            Camera.main.GetComponent<MainScript>().dropDownTime < Time.time)
        {
    ...
    public void checkNextNearestColor(ArrayList b, int counter)
    {
        ...
        {
            if (obj1.gameObject.CompareTag(tag))
            {
    ...
    public void checkNearestColor()
    {
        ...
        GameObject[]
            fixedBalls =
                GameObject.FindObjectsOfType(typeof(GameObject)) as GameObject[]; // change color tag of the rainbow
    ...
    public void StartFall()
    {
        ...
        gameObject.GetComponent<Rigidbody2D>().velocity =
            gameObject.GetComponent<Rigidbody2D>().velocity + new Vector2(Random.Range(-2, 2), 0);
    ...
    IEnumerator FlyToTarget()
    {
        ...
        AnimationCurve curveX =
            new AnimationCurve(new Keyframe(0, transform.position.x), new Keyframe(0.5f, targetPos.x));
        AnimationCurve curveY =
            new AnimationCurve(new Keyframe(0, transform.position.y), new Keyframe(0.5f, targetPos.y));
    ...
    public bool checkNearestBall(ArrayList b)
    {
        if ((MainScript.Instance.TopBorder.transform.position.y - transform.position.y <= 0.5f &&
             LevelData.mode != ModeGame.Rounded) || (LevelData.mode == ModeGame.Rounded && tag == "chicken"))
        {
    ...
    public IEnumerator PlayHitAnimCor(Vector3 newBallPos, float force, Hashtable animTable)
    {
        ...
        if (CompareTag("chicken")) yield break;
        yield return new WaitForFixedUpdate();
    ...
    public void FindLight(GameObject activatedByBall)
    {
        ...
        foreach (Collider2D obj in fixedBalls)
        {
            ...
            if (i <= 10)
            {
                if ((obj.gameObject.CompareTag("light")) && GamePlay.Instance.GameStatus == GameState.Playing)
                {
                    DestroySingle(obj.gameObject);
                    DestroySingle(activatedByBall);
                }
                else if ((obj.gameObject.CompareTag("cloud")) && GamePlay.Instance.GameStatus == GameState.Playing)
                {
    ...
    void OnTriggerEnter2D(Collider2D other)
    {
        ...
        if (other.gameObject.name.Contains("ball") && setTarget && name.IndexOf("bug") < 0)
        {
            ...
            {
                if ((other.gameObject.CompareTag("black_hole")) && GamePlay.Instance.GameStatus == GameState.Playing)
                {
                ...
                {
                    if (other.gameObject.tag.Contains("animal") || other.gameObject.tag.Contains("empty") ||
                        other.gameObject.tag.Contains("chicken")) return;
                    fireBallLimit--;
                    if (fireBallLimit > 0)
    ...
    void DestroyAround()
    {
        ...
        {
            GameObject obj = obj1.gameObject;
            if (!findInArray(b, obj) && obj.tag != "chicken" && !obj.tag.Contains("animal") &&
                !obj.tag.Contains("empty"))
            {
                b.Add(obj);
            }
        }
    ...
    void DestroyLine()
    {
        ...
        ArrayList b = new ArrayList();
        int layerMask = 1 << LayerMask.NameToLayer("Ball");
        RaycastHit2D[] fixedBalls = Physics2D.LinecastAll(transform.position + Vector3.left * 10,
            transform.position + Vector3.right * 10, layerMask);
        int i = 0;
    ...
    {
        if (Physics2D.OverlapCircle(transform.position, 0.1f, 1 << 14) != null ||
            Physics2D.OverlapCircle(transform.position, 0.1f, 1 << 17) != null)
        {
            DestroySingle(gameObject, 0.00001f);
        }
...
//             check if we collided with a right block and adjust our speed and rotation accordingly
    ...
    IEnumerator explode()
    {
        ...
        GameObject explosion = (GameObject) Instantiate(prefab, gameObject.transform.position + Vector3.back * 20f,
            Quaternion.identity);
        if (mesh != null)
    ...
    void CheckNearCloud()
    {
        ...
        foreach (Collider2D obj1 in meshes)
        {
            if (obj1.gameObject.CompareTag("cloud"))
            {
                GameObject obj = obj1.gameObject;
                float distTemp = Vector3.Distance(transform.position, obj.transform.position);
                if (distTemp <= 1f)
